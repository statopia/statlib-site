# Proof narrative — weyl_maxeig_upper

Root: **weyl_maxeig_upper** (lemma) `Statlib/HighDim/SpectralPerturbation.lean:75` · topic `HighDim`
Closure: 2 declarations across 1 files. Generated from `proof_graph.json` — no files were moved.

Reading order (foundations first, headline last):

  ◆ `sortedEigenvalues` — noncomputable def · `Statlib/HighDim/SpectralPerturbation.lean:40`  _(also used by 5: sortedEigenvalues_mono, sortedEigenvalues_perm, weyl_mineig_lower, …)_
· `weyl_maxeig_upper` — lemma · `Statlib/HighDim/SpectralPerturbation.lean:75` **← headline**

## Dependency diagram

```mermaid
flowchart TD
  n0["◆ sortedEigenvalues"]
  n1["· weyl_maxeig_upper"]
  n1 --> n0
  class n1 headline;
  classDef headline fill:#ffe08a,stroke:#d4a017,stroke-width:2px;
  classDef axiomNode fill:#ffd0d0,stroke:#c0392b,stroke-dasharray:4 2;
```
